...
# Vector image extensions copied through without raster optimization.
PASSTHROUGH_SUFFIXES: set[str] = {".svg"}
# Matches Markdown image syntax and optional Pandoc attribute blocks.
_MARKDOWN_IMAGE_RE = re.compile(
    r"!\[(?P<alt>[^\]]*)\]\((?P<target>[^)]+)\)(?P<attrs>\{[^}\n]*\})?"
)
# Detects Windows absolute paths before local image resolution.
...
# Optimization presets keyed by output profile.
