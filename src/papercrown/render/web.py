...
def write_web_stylesheet_bundle(
    ...
    *,
    ...
    sources.extend(
        _web_stylesheet_source(path, theme=theme, theme_out=theme_out)
        for path in theme.css_files
    )
    sections = ["/* Generated by Paper Crown. Edit source CSS, not this file. */"]
    for source in sources:
        label = _web_stylesheet_label(source, styles_dir=styles_dir)
...
def _web_stylesheet_source(
    path: Path,
    *,
    theme: themes.ThemePack,
    theme_out: Path,
) -> Path:
    """Return the copied stylesheet path when it belongs to the theme root."""
    try:
        return theme_out / path.relative_to(theme.root)
    except ValueError:
        return path


def _web_stylesheet_label(source: Path, *, styles_dir: Path) -> str:
    """Return a stable bundle comment label for copied and source CSS files."""
    try:
